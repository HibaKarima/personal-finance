...
function Pots() {
  return (
    <div className="h-auto bg-white rounded-2xl p-5 shadow-sm">
     <Title title="Pots" link="/dashboard/pots" text="See Details"/>
      <div className="flex items-center gap-2 md:justify-evenly">
        ...
        <div className="grid grid-cols-2 lg:gap-x-10 lg:gap-y-5 gap-3">
          {potData.map((item: potItem, index: number) => {
            return (
              <div key={index} className="flex items-center gap-3">
                <div className={`w-1 rounded h-full ${item.color}`} style={{"backgroundColor":`${item.color}`}}></div>
                <div>
